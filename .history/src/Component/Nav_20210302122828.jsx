...
class Nav extends Component {
	...
	divPosition = (e) => {
		...
		if (pos > hero && pos <= profile) {
			this.addClass(1)
		} else if (pos > profile && pos <= portfolio) {
			this.addClass(2)
		} else if (pos > portfolio && pos <= professional) {
			this.addClass(3)
		} else if (pos > professional && pos <= experience) {
			this.addClass(4)
		...
		return (
			<div>
				...
				<ul className={this.state.isOpen ? 'navList' : 'navList navListClose'}>
					...
					<li className="eachNavList" id="tag3" onClick={() => this.scrollToAnchor('portfolio')}>
						PORTFOLIO
					</li>
					<li className="eachNavList" id="tag4" onClick={() => this.scrollToAnchor('professional')}>
						PROFESSIONAL
					</li>
